...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      const account_object& init = db->get_account( KNOWLEDGR_INIT_MINER_NAME );
      asset init_starting_balance = init.balance;

      account_create_operation op;

      op.new_account_name = "alice";
      op.creator = KNOWLEDGR_INIT_MINER_NAME;
...
//       BOOST_REQUIRE( acct.vesting_withdraw_rate.amount.value == ASSET( "0.000000 VESTS" ).amount.value );
      ...
      fund( KNOWLEDGR_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );
      vest( KNOWLEDGR_INIT_MINER_NAME, KNOWLEDGR_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );

      BOOST_TEST_MESSAGE( "--- Test account creation with temp account does not set recovery account" );
      op.creator = KNOWLEDGR_TEMP_ACCOUNT;
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      sign( tx, alice_private_key );
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
      tx.signatures.clear();
      sign( tx, active_key );
      sign( tx, active_key );
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_duplicate_sig );
      ...
      tx.operations.push_back( op );
      sign( tx, active_key );
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / KNOWLEDGR_BLOCK_INTERVAL );
      ...
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      sign( tx, bob_private_key );
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when signed by a signature not in the creator's authority" );
      tx.signatures.clear();
      sign( tx, bob_private_key );
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db->head_block_time() + fc::seconds( KNOWLEDGR_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + KNOWLEDGR_CASHOUT_WINDOW_SECONDS );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + KNOWLEDGR_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / KNOWLEDGR_BLOCK_INTERVAL + 1 );
      ...
      {
         o.total_reward_shares2 = knowledgr::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + KNOWLEDGR_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / KNOWLEDGR_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
   try
   {
      ...
      vest( KNOWLEDGR_INIT_MINER_NAME, "alice", ASSET( "1000.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + KNOWLEDGR_MIN_TRANSACTION_EXPIRATION_LIMIT );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_block();
      vote.weight = -1 * KNOWLEDGR_100_PERCENT;
      tx.clear();
      tx.operations.push_back( vote );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      generate_blocks( KNOWLEDGR_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / KNOWLEDGR_BLOCK_INTERVAL );
      ...
      generate_blocks( KNOWLEDGR_CASHOUT_WINDOW_SECONDS / KNOWLEDGR_BLOCK_INTERVAL );
...
//          op.author = "bob";
//          op.permlink = "foo";
//          op.weight = KNOWLEDGR_100_PERCENT;
//          tx.operations.push_back( op );
//          sign( tx, alice_private_key );
// 
//          KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
...
//          int64_t max_vote_denom = ( db->get_dynamic_global_properties().vote_power_reserve_rate * KNOWLEDGR_VOTING_MANA_REGENERATION_SECONDS ) / (60*60*24);
// 
//          BOOST_REQUIRE( alice.last_vote_time == db->head_block_time() );
//          BOOST_REQUIRE( alice_comment.net_rshares.value == ( old_mana - alice.voting_manabar.current_mana ) - KNOWLEDGR_VOTE_DUST_THRESHOLD );
//          BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + KNOWLEDGR_CASHOUT_WINDOW_SECONDS );
//          BOOST_REQUIRE( itr->rshares == ( old_mana - alice.voting_manabar.current_mana ) - KNOWLEDGR_VOTE_DUST_THRESHOLD );
//          BOOST_REQUIRE( itr != vote_idx.end() );
//          validate_database();
// 
//          BOOST_TEST_MESSAGE( "--- Test reduced power for quick voting" );
// 
//          generate_blocks( db->head_block_time() + KNOWLEDGR_MIN_VOTE_INTERVAL_SEC );
// 
//          util::manabar old_manabar = db->get_account( "alice" ).voting_manabar;
//          util::manabar_params params( util::get_effective_vesting_shares( db->get_account( "alice" ) ), KNOWLEDGR_VOTING_MANA_REGENERATION_SECONDS );
...
//          db->push_transaction( tx, 0 );
// 
//          op.weight = KNOWLEDGR_100_PERCENT / 2;
//          op.voter = "alice";
//          op.author = "bob";
//          op.permlink = "foo";
//          tx.operations.clear();
...
// 
//          BOOST_REQUIRE( bob_comment.net_rshares.value == ( old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana ) - KNOWLEDGR_VOTE_DUST_THRESHOLD );
//          BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + KNOWLEDGR_CASHOUT_WINDOW_SECONDS );
//          BOOST_REQUIRE( itr != vote_idx.end() );
//          validate_database();
// 
...
// 
//          generate_blocks( db->head_block_time() + fc::seconds( ( KNOWLEDGR_CASHOUT_WINDOW_SECONDS / 2 ) ), true );
// 
...
// 
//          BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + ( old_mana - new_bob.voting_manabar.current_mana ) - KNOWLEDGR_VOTE_DUST_THRESHOLD );
//          BOOST_REQUIRE( new_alice_comment.cashout_time == new_alice_comment.created + KNOWLEDGR_CASHOUT_WINDOW_SECONDS );
//          BOOST_REQUIRE( itr != vote_idx.end() );
//          validate_database();
// 
...
//          old_manabar.regenerate_mana( params, db->head_block_time() );
// 
//          op.weight = -1 * KNOWLEDGR_100_PERCENT / 2;
//          op.voter = "sam";
//          op.author = "bob";
//          op.permlink = "foo";
//          tx.operations.clear();
...
//          int64_t sam_weight = old_manabar.current_mana - db->get_account( "sam" ).voting_manabar.current_mana - KNOWLEDGR_VOTE_DUST_THRESHOLD;
// 
...
//          BOOST_REQUIRE( new_bob_comment.cashout_time == new_bob_comment.created + KNOWLEDGR_CASHOUT_WINDOW_SECONDS );
//          BOOST_REQUIRE( itr != vote_idx.end() );
//          validate_database();
// 
...
//          int64_t regenerated_power = (KNOWLEDGR_100_PERCENT * ( db->head_block_time() - db->get_account( "alice").last_vote_time ).to_seconds() ) / KNOWLEDGR_VOTING_MANA_REGENERATION_SECONDS;
...
//          db->push_transaction( tx, 0 );
// 
//          int64_t new_rshares = ( ( fc::uint128_t( db->get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / KNOWLEDGR_100_PERCENT ).to_uint64() - KNOWLEDGR_VOTE_DUST_THRESHOLD;
// 
//          BOOST_REQUIRE( db->get_comment( "alice", string( "foo" ) ).cashout_time == db->get_comment( "alice", string( "foo" ) ).created + KNOWLEDGR_CASHOUT_WINDOW_SECONDS );
// 
//          validate_database();
// 
...
//          old_abs_rshares = new_bob_comment.abs_rshares.value;
//          used_power = ( ( KNOWLEDGR_1_PERCENT * 25 * ( get_voting_power( new_alice ) ) / KNOWLEDGR_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
...
// 
//          op.voter = "alice";
//          op.weight = KNOWLEDGR_1_PERCENT * 25;
//          op.author = "bob";
//          op.permlink = "foo";
//          tx.operations.clear();
//          tx.signatures.clear();
...
// 
//          new_rshares = old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana - KNOWLEDGR_VOTE_DUST_THRESHOLD;
// 
...
//          old_abs_rshares = new_bob_comment.abs_rshares.value;
//          used_power = ( uint64_t( KNOWLEDGR_1_PERCENT ) * 75 * uint64_t( alice_voting_power ) ) / KNOWLEDGR_100_PERCENT;
...
//          old_manabar.regenerate_mana( params, db->head_block_time() );
// 
//          op.weight = KNOWLEDGR_1_PERCENT * -75;
//          tx.operations.clear();
//          tx.signatures.clear();
//          tx.operations.push_back( op );
//          sign( tx, alice_private_key );
...
// 
//          generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - KNOWLEDGR_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + KNOWLEDGR_BLOCK_INTERVAL ), true );
// 
...
//          db->push_transaction( tx, 0 );
// 
//          new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - KNOWLEDGR_VOTE_DUST_THRESHOLD;
//          new_rshares = ( new_rshares * ( KNOWLEDGR_UPVOTE_LOCKOUT_SECONDS - KNOWLEDGR_BLOCK_INTERVAL ) ) / KNOWLEDGR_UPVOTE_LOCKOUT_SECONDS;
...
//          old_manabar.regenerate_mana( params, db->head_block_time() );
// 
//          op.weight = -1 * KNOWLEDGR_100_PERCENT;
//          tx.operations.clear();
//          tx.signatures.clear();
//          tx.operations.push_back( op );
//          sign( tx, dave_private_key );
...
// 
//          new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - KNOWLEDGR_VOTE_DUST_THRESHOLD;
//          new_rshares = ( new_rshares * ( KNOWLEDGR_UPVOTE_LOCKOUT_SECONDS - KNOWLEDGR_BLOCK_INTERVAL - KNOWLEDGR_BLOCK_INTERVAL ) ) / KNOWLEDGR_UPVOTE_LOCKOUT_SECONDS;
// 
//          {
//             auto dave_bob_vote = db->get< comment_vote_object, by_comment_voter >( boost::make_tuple( bob_comment_id, dave_id ) );
//             BOOST_REQUIRE( dave_bob_vote.rshares = new_rshares );
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
   try
   {
      ...
      tx.set_expiration( db->head_block_time() + KNOWLEDGR_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::exception );
...
// 
//       BOOST_TEST_MESSAGE( "--- Test failure when no signature." );
//       KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_active_auth );
// 
...
//       sign( tx, alice_private_key );
//       KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_duplicate_sig );
// 
...
//       BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( KNOWLEDGR_VESTING_WITHDRAW_INTERVALS * 2 ) ).value );
//       BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
//       BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + KNOWLEDGR_VESTING_WITHDRAW_INTERVAL_SECONDS );
//       validate_database();
// 
...
// 
//       BOOST_REQUIRE( alice.vesting_shares.amount.value == old_vesting_shares.amount.value );
//       BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( KNOWLEDGR_VESTING_WITHDRAW_INTERVALS * 3 ) ).value );
//       BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
//       BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + KNOWLEDGR_VESTING_WITHDRAW_INTERVAL_SECONDS );
//       validate_database();
// 
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.proxy == KNOWLEDGR_PROXY_TO_SELF_ACCOUNT );
//      BOOST_REQUIRE( alice.proxied_vsf_votes_total() == bob.vesting_shares.amount );
      ...
      BOOST_REQUIRE( sam.proxy == KNOWLEDGR_PROXY_TO_SELF_ACCOUNT );
...
//      BOOST_REQUIRE( sam.proxied_vsf_votes_total() == bob.vesting_shares.amount );
      BOOST_REQUIRE( dave.proxy == KNOWLEDGR_PROXY_TO_SELF_ACCOUNT );
...
//       BOOST_REQUIRE( dave.proxied_vsf_votes_total() == ( sam.vesting_shares + bob.vesting_shares + alice.vesting_shares ).amount );
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = KNOWLEDGR_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( alice.proxy == "sam" );
      BOOST_REQUIRE( alice.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( bob.proxy == KNOWLEDGR_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE( bob.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( sam.proxy == "dave" );
...
//       BOOST_REQUIRE( dave.proxied_vsf_votes_total() == ( sam.vesting_shares + alice.vesting_shares ).amount );
      ...
      vote.account= "bob";
      vote.witness = KNOWLEDGR_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      sign( tx, bob_private_key );
...
//      BOOST_REQUIRE( db->get_witness( KNOWLEDGR_INIT_MINER_NAME ).votes == ( alice.vesting_shares + bob.vesting_shares ).amount );
      validate_database();
...
//      BOOST_REQUIRE( db->get_witness( KNOWLEDGR_INIT_MINER_NAME ).votes == bob.vesting_shares.amount );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
//       BOOST_REQUIRE( convert_request->conversion_date == db->head_block_time() + KNOWLEDGR_CONVERSION_DELAY );
// 
...
// 
//       fund( "dany", 5000 );
//       KNOWLEDGR_REQUIRE_THROW( convert( "dany", ASSET( "5000.000 TESTS" ) ), fc::exception );
//    }
//    FC_LOG_AND_RETHROW()
// 
// }
...
BOOST_AUTO_TEST_CASE( limit_order_create_apply )
{
   try
   {
      ...
      op.expiration = db->head_block_time() + fc::seconds( KNOWLEDGR_MAX_LIMIT_ORDER_EXPIRATION + 1 );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
...
BOOST_AUTO_TEST_CASE( limit_order_create2_apply )
{
   try
   {
      ...
      /// First check validation on price constructor level:
      {
        price broken_price;
        /// Invalid base value
        KNOWLEDGR_REQUIRE_THROW(broken_price=price(ASSET("0.000 TESTS"), ASSET("1.000 TBD")),
          fc::exception);
        /// Invalid quote value
        KNOWLEDGR_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TBD")),
          fc::exception);
        /// Invalid symbol (same in base & quote)
        KNOWLEDGR_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TESTS")),
          fc::exception);
      }
...
BOOST_AUTO_TEST_CASE( limit_order_cancel_authorities )
{
   try
   {
      ...
      c.expiration = db->head_block_time() + fc::seconds( KNOWLEDGR_MAX_LIMIT_ORDER_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( limit_order_cancel_apply )
{
   try
   {
      ...
      create.expiration = db->head_block_time() + fc::seconds( KNOWLEDGR_MAX_LIMIT_ORDER_EXPIRATION );
      tx.operations.clear();
      tx.signatures.clear();
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      generate_blocks( db->head_block_time() + KNOWLEDGR_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db->head_block_time() + KNOWLEDGR_OWNER_UPDATE_LIMIT + fc::seconds( KNOWLEDGR_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db->head_block_time() + KNOWLEDGR_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - KNOWLEDGR_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db->head_block_time() + ( KNOWLEDGR_OWNER_AUTH_RECOVERY_PERIOD - KNOWLEDGR_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      KNOWLEDGR_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      KNOWLEDGR_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      KNOWLEDGR_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db->head_block_time() + KNOWLEDGR_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( KNOWLEDGR_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      KNOWLEDGR_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      KNOWLEDGR_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
   try
   {
      ...
      auto sam_knowledgr_balance = sam.balance;
      ...
      BOOST_REQUIRE( alice.balance == alice_knowledgr_balance );
      //BOOST_REQUIRE( alice.sbd_balance == alice_sbd_balance );
      BOOST_REQUIRE( bob.balance == bob_knowledgr_balance );
      //BOOST_REQUIRE( bob.sbd_balance == bob_sbd_balance );
      BOOST_REQUIRE( sam.balance == sam_knowledgr_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      tx.set_expiration( db->head_block_time() + KNOWLEDGR_BLOCK_INTERVAL );
      sign( tx, bob_private_key );
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      ...
      KNOWLEDGR_REQUIRE_THROW( db->get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + KNOWLEDGR_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when knowledgr == 0 and sbd == 0" );
      //op.sbd_amount.amount = 0;
      KNOWLEDGR_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      KNOWLEDGR_REQUIRE_THROW( db->get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).complete == db->head_block_time() + KNOWLEDGR_SAVINGS_WITHDRAW_TIME );
      validate_database();
      ...
      generate_blocks( db->head_block_time() + KNOWLEDGR_SAVINGS_WITHDRAW_TIME - fc::seconds( KNOWLEDGR_BLOCK_INTERVAL ), true );
      ...
      for( int i = 0; i < KNOWLEDGR_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = KNOWLEDGR_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == KNOWLEDGR_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db->head_block_time() + KNOWLEDGR_OWNER_AUTH_RECOVERY_PERIOD );
      ...
      vote.weight = KNOWLEDGR_1_PERCENT * 50;
      tx.clear();
      tx.operations.push_back( vote );
      sign( tx, alice_private_key );
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      op.account = "alice";
      op.reward_knowledgr = ASSET( "0.000 TESTS" );
...
//       op.reward_vests = ASSET( "0.000000 VESTS" );
      ...
      op.reward_knowledgr.amount = 1000;
      op.validate();

      op.reward_knowledgr.amount = 0;
      ...
      op.reward_knowledgr = ASSET( "1.000 TESTS" );
...
//            gpo.pending_rewarded_vesting_nlg += ASSET( "10.000 TESTS" );
      ...
      auto alice_knowledgr = db->get_account( "alice" ).balance;
      ...
      op.account = "alice";
      op.reward_knowledgr = ASSET( "20.000 TESTS" );
      op.reward_sbd = ASSET( "0.000 TBD" );
      op.reward_vests = ASSET( "0.000000 VESTS" );
...
//       BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_nlg == ASSET( "5.000 TESTS" ) );
      ...
      op.reward_knowledgr = ASSET( "10.000 TESTS" );
...
//       util::manabar_params params( util::get_effective_vesting_shares( db->get_account( "bob" ) ), KNOWLEDGR_VOTING_MANA_REGENERATION_SECONDS );
//       old_manabar.regenerate_mana( params, db->head_block_time() );
// 
//       comment_operation comment_op;
//       comment_op.author = "alice";
...
//       BOOST_REQUIRE( alice_comment.net_rshares.value == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - KNOWLEDGR_VOTE_DUST_THRESHOLD );
//       BOOST_REQUIRE( itr->rshares == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - KNOWLEDGR_VOTE_DUST_THRESHOLD );
// 
//       generate_block();
//       ACTORS( (sam)(dave) )
//       generate_block();
// 
//       vest( KNOWLEDGR_INIT_MINER_NAME, "sam", ASSET( "1000.000 TESTS" ) );
// 
//       generate_block();
// 
...
//       tx.set_expiration( db->head_block_time() + KNOWLEDGR_MAX_TIME_UNTIL_EXPIRATION );
//       sign( tx, sam_private_key );
//       KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
// 
// 
...
//       auto gpo = db->get_dynamic_global_properties();
// 
//       BOOST_REQUIRE( gpo.delegation_return_period == KNOWLEDGR_DELEGATION_RETURN_PERIOD_HF20 );
// 
...
//       BOOST_REQUIRE( delegation == nullptr );
// 
//       generate_blocks( exp_obj->expiration + KNOWLEDGR_BLOCK_INTERVAL );
// 
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_validate )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), KNOWLEDGR_100_PERCENT + 1 ) );
      op.extensions.insert( b );
      KNOWLEDGR_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- Testing more than 100% total weight" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), KNOWLEDGR_1_PERCENT * 75 ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), KNOWLEDGR_1_PERCENT * 75 ) );
      op.extensions.clear();
      op.extensions.insert( b );
      KNOWLEDGR_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", KNOWLEDGR_1_PERCENT * 2 ) );
      b.beneficiaries.push_back( beneficiary_route_type( "bob", KNOWLEDGR_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      KNOWLEDGR_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", KNOWLEDGR_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( "alice", KNOWLEDGR_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      KNOWLEDGR_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), KNOWLEDGR_1_PERCENT ) );

      for( size_t i = 0; i < 8; i++ )
      {
         b.beneficiaries.push_back( beneficiary_route_type( account_name_type( KNOWLEDGR_INIT_MINER_NAME + fc::to_string( i ) ), KNOWLEDGR_1_PERCENT ) );
      }
      ...
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx ), chain::plugin_exception );


      BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), KNOWLEDGR_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.clear();
      tx.operations.push_back( op );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * KNOWLEDGR_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * KNOWLEDGR_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * KNOWLEDGR_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      sign( tx, alice_private_key );
      KNOWLEDGR_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - KNOWLEDGR_BLOCK_INTERVAL );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
         {
            gpo.current_supply -= gpo.total_reward_fund_knowledgr;
            gpo.total_reward_fund_knowledgr = ASSET( "100.000 TESTS" );
            gpo.current_supply += gpo.total_reward_fund_knowledgr;
         });
      });
...
BOOST_AUTO_TEST_CASE( witness_set_properties_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting maximum_block_size below KNOWLEDGR_MIN_BLOCK_SIZE_LIMIT" );
      prop_op.props.erase( "account_creation_fee" );
      prop_op.props[ "maximum_block_size" ] = fc::raw::pack_to_vector( KNOWLEDGR_MIN_BLOCK_SIZE_LIMIT - 1 );
      KNOWLEDGR_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
...
//       BOOST_TEST_MESSAGE( "--- failure when setting sbd_interest_rate to KNOWLEDGR_100_PERCENT + 1" );
...
//       KNOWLEDGR_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      uint64_t unit = uint64_t(1) << KNOWLEDGR_RD_DECAY_DENOM_SHIFT;

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one year" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / KNOWLEDGR_BLOCKS_PER_YEAR ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one day" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / KNOWLEDGR_BLOCKS_PER_DAY ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one hour" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / ((60*60)/KNOWLEDGR_BLOCK_INTERVAL) ) );
      prop_op.validate();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_authorities )
{
   try
   {
      ...
      expected_keys.push_back( authority( 1, KNOWLEDGR_NULL_ACCOUNT, 1 ) );
      op.get_required_authorities( key_auths );
      BOOST_REQUIRE( key_auths == expected_keys );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_apply )
{
   try
   {
      ...
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.maximum_block_size == KNOWLEDGR_MIN_BLOCK_SIZE_LIMIT + 1 );
...
//      prop_op.props.erase( "sbd_exchange_rate" );
      ...
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "account_subsidy_budget" ] = fc::raw::pack_to_vector( KNOWLEDGR_ACCOUNT_SUBSIDY_PRECISION );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.account_subsidy_budget == KNOWLEDGR_ACCOUNT_SUBSIDY_PRECISION );

      BOOST_TEST_MESSAGE( "--- Testing setting account subsidy pool cap" );
      uint64_t day_decay = ( uint64_t(1) << KNOWLEDGR_RD_DECAY_DENOM_SHIFT ) / KNOWLEDGR_BLOCKS_PER_DAY;
...
BOOST_AUTO_TEST_CASE( claim_account_apply )
{
   try
   {
      ...
      generate_blocks( KNOWLEDGR_BLOCKS_PER_HOUR/2 );
      ...
      BOOST_REQUIRE( db->get_account( KNOWLEDGR_NULL_ACCOUNT ).balance == ASSET( "5.000 TESTS" ) );
      ...
      BOOST_CHECK_EQUAL( c_subs, prev_c_subs - KNOWLEDGR_ACCOUNT_SUBSIDY_PRECISION );
      BOOST_CHECK_EQUAL( nc_subs, prev_nc_subs );
      ...
      int64_t new_value = prev_c_subs - KNOWLEDGR_ACCOUNT_SUBSIDY_PRECISION;     // Usage applied before decay
      new_value = new_value
          + 5000                                                             // Budget
          - ((new_value*249617279) >> KNOWLEDGR_RD_DECAY_DENOM_SHIFT);           // Decay
...
BOOST_AUTO_TEST_CASE( create_claimed_account_apply )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( KNOWLEDGR_TEMP_ACCOUNT ), [&]( account_object& a )
         {
