...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      const account_object& init = db->get_account( COLAB_INIT_MINER_NAME );
      asset init_starting_balance = init.balance;

      account_create_operation op;

      op.new_account_name = "alice";
      op.creator = COLAB_INIT_MINER_NAME;
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db->get_account( COLAB_INIT_MINER_NAME ).balance.amount + 1, CLC_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      sign( tx, init_account_priv_key );
      ...
      fund( COLAB_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );
      vest( COLAB_INIT_MINER_NAME, COLAB_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );

      BOOST_TEST_MESSAGE( "--- Test account creation with temp account does not set recovery account" );
      op.creator = COLAB_TEMP_ACCOUNT;
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      tx.operations.push_back( op );
      sign( tx, active_key );
      COLAB_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / COLAB_BLOCK_INTERVAL );
      ...
      COLAB_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      tx.signatures.clear();
      sign( tx, bob_private_key );
      COLAB_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db->head_block_time() + fc::seconds( COLAB_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + COLAB_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / COLAB_BLOCK_INTERVAL + 1 );
      ...
      {
         o.total_reward_shares2 = colab::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + COLAB_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / COLAB_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
   try
   {
      ...
      tx.set_expiration( db->head_block_time() + COLAB_MIN_TRANSACTION_EXPIRATION_LIMIT );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_block();
      vote.weight = -1 * COLAB_100_PERCENT;
      tx.clear();
      tx.operations.push_back( vote );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      generate_blocks( COLAB_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / COLAB_BLOCK_INTERVAL );
      ...
      generate_blocks( COLAB_CASHOUT_WINDOW_SECONDS / COLAB_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = legacy_colab_asset::from_asset( asset(COLAB_MIN_ACCOUNT_CREATION_FEE + 10, CLC_SYMBOL) );
      op.props.maximum_block_size = COLAB_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = COLAB_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      ...
      vote.account= "bob";
      vote.witness = COLAB_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      sign( tx, bob_private_key );
...
BOOST_AUTO_TEST_CASE( limit_order_create_apply )
{
   try
   {
      ...
      op.expiration = db->head_block_time() + fc::seconds( COLAB_MAX_LIMIT_ORDER_EXPIRATION + 1 );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( SBD_SYMBOL, CLC_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( limit_order_create2_apply )
{
   try
   {
      ...
      /// First check validation on price constructor level:
      {
        price broken_price;
        /// Invalid base value
        COLAB_REQUIRE_THROW(broken_price=price(ASSET("0.000 TESTS"), ASSET("1.000 TBD")),
          fc::exception);
        /// Invalid quote value
        COLAB_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TBD")),
          fc::exception);
        /// Invalid symbol (same in base & quote)
        COLAB_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TESTS")),
          fc::exception);
      }
...
BOOST_AUTO_TEST_CASE( limit_order_cancel_authorities )
{
   try
   {
      ...
      c.expiration = db->head_block_time() + fc::seconds( COLAB_MAX_LIMIT_ORDER_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( limit_order_cancel_apply )
{
   try
   {
      ...
      create.expiration = db->head_block_time() + fc::seconds( COLAB_MAX_LIMIT_ORDER_EXPIRATION );
      tx.operations.clear();
      tx.signatures.clear();
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      vest( COLAB_INIT_MINER_NAME, "bob", asset( 1000, CLC_SYMBOL ) );
      ...
      generate_blocks( db->head_block_time() + COLAB_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db->head_block_time() + COLAB_OWNER_UPDATE_LIMIT + fc::seconds( COLAB_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db->head_block_time() + COLAB_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - COLAB_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db->head_block_time() + ( COLAB_OWNER_AUTH_RECOVERY_PERIOD - COLAB_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      COLAB_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      COLAB_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      COLAB_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db->head_block_time() + COLAB_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( COLAB_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      COLAB_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      COLAB_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when fee symbol != SBD and fee symbol != CLC" );
      op.clc_amount.symbol = CLC_SYMBOL;
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
   try
   {
      ...
      auto alice_colab_balance = alice.balance - op.clc_amount - op.fee;
      //auto alice_sbd_balance = alice.sbd_balance - op.sbd_amount;
      auto bob_colab_balance = bob.balance;
      //auto bob_sbd_balance = bob.sbd_balance;
      auto sam_colab_balance = sam.balance;
      ...
      BOOST_REQUIRE( escrow.clc_balance == op.clc_amount );
      ...
      BOOST_REQUIRE( alice.balance == alice_colab_balance );
      //BOOST_REQUIRE( alice.sbd_balance == alice_sbd_balance );
      BOOST_REQUIRE( bob.balance == bob_colab_balance );
      //BOOST_REQUIRE( bob.sbd_balance == bob_sbd_balance );
      BOOST_REQUIRE( sam.balance == sam_colab_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      et_op.to = "bob";
      et_op.agent = "sam";
      et_op.clc_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( escrow.clc_balance == ASSET( "1.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + COLAB_BLOCK_INTERVAL );
      sign( tx, bob_private_key );
      COLAB_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      ...
      COLAB_REQUIRE_THROW( db->get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + COLAB_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
   try
   {
      ...
      et_op.clc_amount = ASSET( "1.000 TESTS" );
      et_op.fee = ASSET( "0.100 TESTS" );
      et_op.ratification_deadline = db->head_block_time() + COLAB_BLOCK_INTERVAL;
      et_op.escrow_expiration = db->head_block_time() + 2 * COLAB_BLOCK_INTERVAL;
      ...
      BOOST_REQUIRE( escrow.clc_balance == et_op.clc_amount );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).clc_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).clc_balance == ASSET( "0.800 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).clc_balance == ASSET( "0.700 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).clc_balance == ASSET( "0.600 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).clc_balance == ASSET( "0.500 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- success deleting escrow when balances are both zero" );
      tx.clear();
      op.clc_amount = ASSET( "0.500 TESTS" );
      tx.operations.push_back( op );
      sign( tx, sam_private_key );
      db->push_transaction( tx, 0 );

      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "9.700 TESTS" ) );
      COLAB_REQUIRE_THROW( db->get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).clc_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).clc_balance == ASSET( "0.800 TESTS" ) );
      ...
      tx.clear();
      op.clc_amount = ASSET( "0.600 TESTS" );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "success when amount is CLC" );
      op.amount = ASSET( "1.000 TESTS" );
      op.validate();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success transferring CLC to self" );
      op.to = "alice";
      ...
      BOOST_TEST_MESSAGE( "--- success transferring CLC to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing CLC to self" );
      op.to = "alice";
      ...
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).complete == db->head_block_time() + COLAB_SAVINGS_WITHDRAW_TIME );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing CLC to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id = 3;
      ...
      generate_blocks( db->head_block_time() + COLAB_SAVINGS_WITHDRAW_TIME - fc::seconds( COLAB_BLOCK_INTERVAL ), true );
      ...
      for( int i = 0; i < COLAB_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = COLAB_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      COLAB_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == COLAB_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db->head_block_time() + COLAB_OWNER_AUTH_RECOVERY_PERIOD );
      ...
      vote.weight = COLAB_1_PERCENT * 50;
      tx.clear();
      tx.operations.push_back( vote );
      sign( tx, alice_private_key );
      COLAB_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      op.reward_colab = ASSET( "1.000 TBD" );
      COLAB_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
//            gpo.current_sbd_supply += ASSET( "10.000 TBD" );
      ...
      op.account = "alice";
      op.reward_colab = ASSET( "20.000 TESTS" );
      op.reward_sbd = ASSET( "0.000 TBD" );
      op.reward_vests = ASSET( "0.000000 VESTS" );
...
//       BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_clc == ASSET( "5.000 TESTS" ) );
      ...
      op.reward_colab = ASSET( "10.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_validate )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), COLAB_100_PERCENT + 1 ) );
      op.extensions.insert( b );
      COLAB_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- Testing more than 100% total weight" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), COLAB_1_PERCENT * 75 ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), COLAB_1_PERCENT * 75 ) );
      op.extensions.clear();
      op.extensions.insert( b );
      COLAB_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", COLAB_1_PERCENT * 2 ) );
      b.beneficiaries.push_back( beneficiary_route_type( "bob", COLAB_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      COLAB_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", COLAB_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( "alice", COLAB_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      COLAB_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), COLAB_1_PERCENT ) );

      for( size_t i = 0; i < 8; i++ )
      {
         b.beneficiaries.push_back( beneficiary_route_type( account_name_type( COLAB_INIT_MINER_NAME + fc::to_string( i ) ), COLAB_1_PERCENT ) );
      }
      ...
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      COLAB_REQUIRE_THROW( db->push_transaction( tx ), chain::plugin_exception );


      BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), COLAB_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.clear();
      tx.operations.push_back( op );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * COLAB_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * COLAB_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * COLAB_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      sign( tx, alice_private_key );
      COLAB_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - COLAB_BLOCK_INTERVAL );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
         {
            gpo.current_supply -= gpo.total_reward_fund_colab;
            gpo.total_reward_fund_colab = ASSET( "100.000 TESTS" );
            gpo.current_supply += gpo.total_reward_fund_colab;
         });
      });
...
BOOST_AUTO_TEST_CASE( witness_set_properties_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting maximum_block_size below COLAB_MIN_BLOCK_SIZE_LIMIT" );
      ...
      uint64_t unit = uint64_t(1) << COLAB_RD_DECAY_DENOM_SHIFT;

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one year" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / COLAB_BLOCKS_PER_YEAR ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one day" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / COLAB_BLOCKS_PER_DAY ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one hour" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / ((60*60)/COLAB_BLOCK_INTERVAL) ) );
      prop_op.validate();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_authorities )
{
   try
   {
      ...
      expected_keys.push_back( authority( 1, COLAB_NULL_ACCOUNT, 1 ) );
      op.get_required_authorities( key_auths );
      BOOST_REQUIRE( key_auths == expected_keys );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_apply )
{
   try
   {
      ...
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "account_subsidy_budget" ] = fc::raw::pack_to_vector( COLAB_ACCOUNT_SUBSIDY_PRECISION );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.account_subsidy_budget == COLAB_ACCOUNT_SUBSIDY_PRECISION );

      BOOST_TEST_MESSAGE( "--- Testing setting account subsidy pool cap" );
      uint64_t day_decay = ( uint64_t(1) << COLAB_RD_DECAY_DENOM_SHIFT ) / COLAB_BLOCKS_PER_DAY;
...
BOOST_AUTO_TEST_CASE( claim_account_apply )
{
   try
   {
      ...
      generate_blocks( COLAB_BLOCKS_PER_HOUR/2 );
      ...
      BOOST_REQUIRE( db->get_account( COLAB_NULL_ACCOUNT ).balance == ASSET( "5.000 TESTS" ) );
      ...
      BOOST_CHECK_EQUAL( c_subs, prev_c_subs - COLAB_ACCOUNT_SUBSIDY_PRECISION );
      BOOST_CHECK_EQUAL( nc_subs, prev_nc_subs );
      ...
      int64_t new_value = prev_c_subs - COLAB_ACCOUNT_SUBSIDY_PRECISION;     // Usage applied before decay
      new_value = new_value
          + 5000                                                             // Budget
          - ((new_value*249617279) >> COLAB_RD_DECAY_DENOM_SHIFT);           // Decay
...
BOOST_AUTO_TEST_CASE( create_claimed_account_apply )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( COLAB_TEMP_ACCOUNT ), [&]( account_object& a )
         {
