...
BOOST_AUTO_TEST_CASE( smt_limit_order_create_authorities )
{
   try
   {
      ...
      op.expiration = db->head_block_time() + fc::seconds( COLAB_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( smt_limit_order_create_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_balance( bob_account, CLC_SYMBOL ).amount.value == bob_balance.amount.value );
      ...
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( alice_symbol, CLC_SYMBOL ) );
      BOOST_REQUIRE( db->get_balance( alice_account, alice_symbol ).amount.value == alice_smt_balance.amount.value );
      BOOST_REQUIRE( db->get_balance( alice_account, CLC_SYMBOL ).amount.value == alice_balance.amount.value );
      validate_database();
...
BOOST_AUTO_TEST_CASE( claim_reward_balance2_apply )
{
   try
   {
      ...
      // Legacy symbols
      asset full_colab = ASSET( "10.000 TESTS" );
      asset full_sbd = ASSET( "10.000 TBD" );
      op.reward_tokens.push_back( full_sbd );
      op.reward_tokens.push_back( full_colab );
      op.reward_tokens.push_back( partial_vests );
      PUSH_OP(op, alice_private_key);
...
BOOST_AUTO_TEST_CASE( smt_create_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( " -- Valid maximum SMT creation fee (COLAB_MAX_SHARE_SUPPLY)" );
      op.smt_creation_fee.amount = COLAB_MAX_SHARE_SUPPLY;
      op.validate();
...
//       op.smt_creation_fee = db->get_dynamic_global_properties().smt_creation_fee;
      ...
      BOOST_TEST_MESSAGE( " -- Invalid SMT creation symbol: CLC cannot be an SMT" );
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate )
{
   try
   {
      ...
      COLAB_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, alice_symbol.to_nai(), alice_symbol.decimals() ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate_differing_decimals )
{
   try
   {
      ...
      // Fail on duplicate SMT lookup
      COLAB_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, alice_symbol.to_nai(), 2 /* Decimals */ ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate_different_users )
{
   try
   {
      ...
      // Fail on duplicate SMT lookup
      COLAB_REQUIRE_THROW( create_smt_with_nai( "bob", bob_private_key, alice_symbol.to_nai(), alice_symbol.decimals() ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}

BOOST_AUTO_TEST_CASE( smt_create_with_colab_funds )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: smt_create_with_colab_funds" );
...
BOOST_AUTO_TEST_CASE( smt_create_with_invalid_nai )
{
   try
   {
      ...
      COLAB_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, ast.to_nai(), ast.decimals() ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_creation_fee_test )
{
   try
   {
      ...
      // This ensures that our actual smt_creation_fee is sane in production (either CLC or SBD)
      const dynamic_global_property_object& dgpo = db->get_dynamic_global_properties();
      FC_ASSERT( dgpo.smt_creation_fee.symbol == CLC_SYMBOL || dgpo.smt_creation_fee.symbol == SBD_SYMBOL,
      ...
      for ( int i = 0; i < 2; i++ )
      {
         ...
         if ( !i ) // First pass
            ...
            {
               dgpo.smt_creation_fee = asset( 2000, CLC_SYMBOL );
            } );
         else // Second pass
            db->modify( dgpo, [&] ( dynamic_global_property_object& dgpo )
            {
         ...
         BOOST_TEST_MESSAGE( " -- Valid creation fee, using CLC" );
         // We should be able to pay with CLC
         smt_create_operation op;
         op.control_account = "alice";
...
BOOST_AUTO_TEST_CASE( smt_nai_pool_count )
{
   try
   {
      ...
      COLAB_REQUIRE_THROW( get_new_smt_symbol( 0, this->db ), fc::assert_exception );
