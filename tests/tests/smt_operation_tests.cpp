...
BOOST_AUTO_TEST_CASE( smt_limit_order_create_authorities )
{
   try
   {
      ...
      op.expiration = db->head_block_time() + fc::seconds( KNOWLEDGR_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance2_apply )
{
   try
   {
      ...
      // Legacy symbols
      asset full_knowledgr = ASSET( "10.000 TESTS" );
      asset full_sbd = ASSET( "10.000 TBD" );
      op.reward_tokens.push_back( full_sbd );
      op.reward_tokens.push_back( full_knowledgr );
      op.reward_tokens.push_back( partial_vests );
      PUSH_OP(op, alice_private_key);
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate )
{
   try
   {
      ...
      KNOWLEDGR_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, alice_symbol.to_nai(), alice_symbol.decimals() ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate_differing_decimals )
{
   try
   {
      ...
      // Fail on duplicate SMT lookup
      KNOWLEDGR_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, alice_symbol.to_nai(), 2 /* Decimals */ ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate_different_users )
{
   try
   {
      ...
      // Fail on duplicate SMT lookup
      KNOWLEDGR_REQUIRE_THROW( create_smt_with_nai( "bob", bob_private_key, alice_symbol.to_nai(), alice_symbol.decimals() ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}

BOOST_AUTO_TEST_CASE( smt_create_with_knowledgr_funds )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: smt_create_with_knowledgr_funds" );
...
BOOST_AUTO_TEST_CASE( smt_create_with_invalid_nai )
{
   try
   {
      ...
      KNOWLEDGR_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, ast.to_nai(), ast.decimals() ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_nai_pool_count )
{
   try
   {
      ...
      KNOWLEDGR_REQUIRE_THROW( get_new_smt_symbol( 0, this->db ), fc::assert_exception );
