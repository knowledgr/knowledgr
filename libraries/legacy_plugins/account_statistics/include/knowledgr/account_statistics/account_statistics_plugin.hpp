...
// time.
//
#ifndef KNOWLEDGR_ACCOUNT_STATISTICS_SPACE_ID
#define KNOWLEDGR_ACCOUNT_STATISTICS_SPACE_ID 10
#endif

#ifndef KNOWLEDGR_ACCOUNT_STATISTICS_PLUGIN_NAME
#define KNOWLEDGR_ACCOUNT_STATISTICS_PLUGIN_NAME "account_stats"
#endif
...
{
   account_stats_bucket_object_type    = ( KNOWLEDGR_ACCOUNT_STATISTICS_SPACE_ID << 8 ),
   account_activity_bucket_object_type = ( KNOWLEDGR_ACCOUNT_STATISTICS_SPACE_ID << 8 ) + 1
};
...
//   share_type           author_rewards_sbd = 0;                   ///< SBD paid for author rewards
   ...
   share_type           author_rewards_total_knlg_value = 0;     ///< KNLG Value of author rewards
//   share_type           author_rewards_payout_sbd_value = 0;      ///< SBD Value of author rewards at time of payout
   ...
   share_type           curation_rewards_knlg_value = 0;         ///< KNLG Value of curation rewards
   ...
   share_type           liquidity_rewards = 0;                    ///< Amount of KNLG paid as liquidity rewards
   ...
   share_type           knlg_sent = 0;                           ///< KNLG sent from this account
   share_type           knlg_received = 0;                       ///< KNLG received by this account
...
//    uint32_t             transfers_to_vesting = 0;                 ///< Transfers to vesting by this account. Note: Transfer to vesting from A to B counts as a transfer from A to B followed by a vesting deposit by B.
   share_type           knlg_vested = 0;                         ///< KNLG vested by the account
...
//   share_type           vests_withdrawn = 0;                      ///< VESTS withdrawn from the account
   share_type           knlg_received_from_withdrawls = 0;       ///< KNLG received from this account's vesting withdrawals
   share_type           knlg_received_from_routes = 0;           ///< KNLG received from another account's vesting withdrawals
...
//    uint32_t             sbd_conversion_requests_filled = 0;       ///< SBD conversion requests filled
   ...
   share_type           limit_order_knlg_paid = 0;               ///< KNLG paid by limit orders
   share_type           limit_order_knlg_received = 0;           ///< KNLG received from limit orders
...
class account_statistics_plugin : public knowledgr::app::plugin
{
   public:
      account_statistics_plugin( application* app );
      virtual ~account_statistics_plugin();

      virtual std::string plugin_name()const override { return KNOWLEDGR_ACCOUNT_STATISTICS_PLUGIN_NAME; }
...
FC_REFLECT( knowledgr::account_statistics::account_stats_bucket_object,
   (id)
   (open)
   (seconds)
   (name)
...
//   (vests_withdrawn)
   (knlg_received_from_withdrawls)
   (knlg_received_from_routes)
...
//    (sbd_conversion_requests_filled)
   ...
   (limit_order_knlg_paid)
   (limit_order_knlg_received)
//    (limit_order_sbd_paid)
//    (limit_order_sbd_received)
   (total_pow)
   (estimated_hashpower)
)
//SET_INDEX_TYPE( knowledgr::account_statistics::account_stats_bucket_object,)

FC_REFLECT(
   knowledgr::account_statistics::account_activity_bucket_object,
   (id)
   (open)
   (seconds)
