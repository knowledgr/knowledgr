...
// time.
//
#ifndef COLAB_ACCOUNT_STATISTICS_SPACE_ID
#define COLAB_ACCOUNT_STATISTICS_SPACE_ID 10
#endif

#ifndef COLAB_ACCOUNT_STATISTICS_PLUGIN_NAME
#define COLAB_ACCOUNT_STATISTICS_PLUGIN_NAME "account_stats"
#endif
...
{
   account_stats_bucket_object_type    = ( COLAB_ACCOUNT_STATISTICS_SPACE_ID << 8 ),
   account_activity_bucket_object_type = ( COLAB_ACCOUNT_STATISTICS_SPACE_ID << 8 ) + 1
};
...
struct account_stats_bucket_object : public object< account_stats_bucket_object_type, account_stats_bucket_object >
{
   ...
   share_type           author_rewards_total_clc_value = 0;     ///< CLC Value of author rewards
   ...
   share_type           curation_rewards_clc_value = 0;         ///< CLC Value of curation rewards
   ...
   share_type           liquidity_rewards = 0;                    ///< Amount of CLC paid as liquidity rewards
   ...
   share_type           clc_sent = 0;                           ///< CLC sent from this account
   share_type           clc_received = 0;                       ///< CLC received by this account
   ...
   share_type           clc_vested = 0;                         ///< CLC vested by the account
   ...
   share_type           clc_received_from_withdrawls = 0;       ///< CLC received from this account's vesting withdrawals
   share_type           clc_received_from_routes = 0;           ///< CLC received from another account's vesting withdrawals
   ...
   share_type           limit_order_clc_paid = 0;               ///< CLC paid by limit orders
   share_type           limit_order_clc_received = 0;           ///< CLC received from limit orders
...
class account_statistics_plugin : public colab::app::plugin
{
   public:
      account_statistics_plugin( application* app );
      virtual ~account_statistics_plugin();

      virtual std::string plugin_name()const override { return COLAB_ACCOUNT_STATISTICS_PLUGIN_NAME; }
...
FC_REFLECT( colab::account_statistics::account_stats_bucket_object,
   (id)
   (open)
   (seconds)
   (name)
   ...
   (vests_withdrawn)
   (clc_received_from_withdrawls)
   (clc_received_from_routes)
   ...
   (limit_order_clc_paid)
   (limit_order_clc_received)
   (limit_order_sbd_paid)
   (limit_order_sbd_received)
   (total_pow)
   (estimated_hashpower)
)
//SET_INDEX_TYPE( colab::account_statistics::account_stats_bucket_object,)

FC_REFLECT(
   colab::account_statistics::account_activity_bucket_object,
   (id)
   (open)
   (seconds)
