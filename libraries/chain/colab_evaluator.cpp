...
{
   FC_ASSERT( permlink.size() > COLAB_MIN_PERMLINK_LENGTH && permlink.size() < COLAB_MAX_PERMLINK_LENGTH, "Permlink is not a valid size." );
...
void witness_update_evaluator::do_apply( const witness_update_operation& o )
{
   ...
   if ( _db.has_hardfork( COLAB_HARDFORK_0_14__410 ) )
   {
      ...
      if( _db.has_hardfork( COLAB_HARDFORK_0_20__2651 ) || _db.is_producing() )
      {
         ...
         FC_ASSERT( o.props.account_creation_fee.amount <= COLAB_MAX_ACCOUNT_CREATION_FEE, "account_creation_fee greater than maximum account creation fee" );
      }
   }
   ...
   if( _db.is_producing() || _db.has_hardfork( COLAB_HARDFORK_0_20__2642 ))
   {
      FC_ASSERT( o.props.maximum_block_size <= COLAB_SOFT_MAX_BLOCK_SIZE, "Max block size cannot be more than 2MiB" );
   }
...
{
   FC_ASSERT( _db.has_hardfork( COLAB_HARDFORK_0_20__1620 ), "witness_set_properties_evaluator not enabled until HF 20" );
   ...
   if( flags.account_creation_changed )
   {
      ...
      {
         FC_TODO( "Move to validate() after HF20" );
         FC_ASSERT( props.account_creation_fee.amount <= COLAB_MAX_ACCOUNT_CREATION_FEE, "account_creation_fee greater than maximum account creation fee" );
      }
   }
...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
   ...
   if( _db.has_hardfork( COLAB_HARDFORK_0_20__1771 ) )
   {
      FC_ASSERT( o.fee == wso.median_props.account_creation_fee, "Must pay the exact account creation fee. paid: ${p} fee: ${f}",
                  ("p", o.fee)
                  ("f", wso.median_props.account_creation_fee) );
   }
   else if( !_db.has_hardfork( COLAB_HARDFORK_0_20__1761 ) && _db.has_hardfork( COLAB_HARDFORK_0_19__987 ) )
   {
      FC_ASSERT( o.fee >= asset( wso.median_props.account_creation_fee.amount * COLAB_CREATE_ACCOUNT_WITH_COLAB_MODIFIER, CLC_SYMBOL ), "Insufficient Fee: ${f} required, ${p} provided.",
                 ("f", wso.median_props.account_creation_fee * asset( COLAB_CREATE_ACCOUNT_WITH_COLAB_MODIFIER, CLC_SYMBOL ) )
                 ("p", o.fee) );
   }
   ...
   if( _db.has_hardfork( COLAB_HARDFORK_0_15__465 ) )
   {
...
{
   if( _db.has_hardfork( COLAB_HARDFORK_0_1 ) ) FC_ASSERT( o.account != COLAB_TEMP_ACCOUNT, "Cannot update temp account." );

   if( ( _db.has_hardfork( COLAB_HARDFORK_0_15__465 ) ) && o.posting )
      o.posting->validate();
   ...
   if( o.owner )
   {
      ...
      if( _db.has_hardfork( COLAB_HARDFORK_0_11 ) )
         FC_ASSERT( _db.head_block_time() - account_auth.last_owner_update > COLAB_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
#endif
   ...
   if( o.active && ( _db.has_hardfork( COLAB_HARDFORK_0_15__465 ) ) )
      verify_authority_accounts_exist( _db, *o.active, o.account, authority::active );
   if( o.posting && ( _db.has_hardfork( COLAB_HARDFORK_0_15__465 ) ) )
...
void delete_comment_evaluator::do_apply( const delete_comment_operation& o )
{
   ...
   if( _db.has_hardfork( COLAB_HARDFORK_0_19__876 ) )
      FC_ASSERT( comment.cashout_time != fc::time_point_sec::maximum() );

   if( _db.has_hardfork( COLAB_HARDFORK_0_19__977 ) )
   ...
   if( _db.has_hardfork( COLAB_HARDFORK_0_6__80 ) && comment.parent_author != COLAB_ROOT_POST_PARENT )
   {
...
void comment_options_evaluator::do_apply( const comment_options_operation& o )
{
   ...
   FC_ASSERT( comment.percent_colab_dollars >= o.percent_colab_dollars, "A comment cannot accept a greater percent SBD." );

   _db.modify( comment, [&]( comment_object& c ) {
       c.max_accepted_payout   = o.max_accepted_payout;
       c.percent_colab_dollars = o.percent_colab_dollars;
...
{ try {
   ...
   if( o.parent_author != COLAB_ROOT_POST_PARENT )
   {
      ...
      if( !_db.has_hardfork( COLAB_HARDFORK_0_17__767 ) )
         FC_ASSERT( parent->depth < COLAB_MAX_COMMENT_DEPTH_PRE_HF17, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",COLAB_MAX_COMMENT_DEPTH_PRE_HF17) );
      else
         FC_ASSERT( parent->depth < COLAB_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",COLAB_MAX_COMMENT_DEPTH) );
   }
   ...
   if ( itr == by_permlink_idx.end() )
   {
      ...
      {
         if( o.parent_author == COLAB_ROOT_POST_PARENT )
             FC_ASSERT( ( now - auth.last_root_post ) > COLAB_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
         else
             FC_ASSERT( (now - auth.last_post) >= COLAB_MIN_REPLY_INTERVAL_HF20, "You may only comment once every 3 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else if( _db.has_hardfork( COLAB_HARDFORK_0_12__176 ) )
      {
         if( o.parent_author == COLAB_ROOT_POST_PARENT )
             FC_ASSERT( ( now - auth.last_root_post ) > COLAB_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
         else
             FC_ASSERT( (now - auth.last_post) > COLAB_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else if( _db.has_hardfork( COLAB_HARDFORK_0_6__113 ) )
      {
         if( o.parent_author == COLAB_ROOT_POST_PARENT )
             FC_ASSERT( (now - auth.last_post) > COLAB_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("auth.last_post",auth.last_post) );
         else
             FC_ASSERT( (now - auth.last_post) > COLAB_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else
      ...
      uint16_t reward_weight = COLAB_100_PERCENT;
      uint64_t post_bandwidth = auth.post_bandwidth;

      if( _db.has_hardfork( COLAB_HARDFORK_0_12__176 ) && !_db.has_hardfork( COLAB_HARDFORK_0_17__733 ) && o.parent_author == COLAB_ROOT_POST_PARENT )
      {
         uint64_t post_delta_time = std::min( _db.head_block_time().sec_since_epoch() - auth.last_root_post.sec_since_epoch(), COLAB_POST_AVERAGE_WINDOW );
         uint32_t old_weight = uint32_t( ( post_bandwidth * ( COLAB_POST_AVERAGE_WINDOW - post_delta_time ) ) / COLAB_POST_AVERAGE_WINDOW );
         post_bandwidth = ( old_weight + COLAB_100_PERCENT );
         reward_weight = uint16_t( std::min( ( COLAB_POST_WEIGHT_CONSTANT * COLAB_100_PERCENT ) / ( post_bandwidth * post_bandwidth ), uint64_t( COLAB_100_PERCENT ) ) );
      }
      ...
      const auto& new_comment = _db.create< comment_object >( [&]( comment_object& com )
      {
         ...
         {
            com.cashout_time = com.created + COLAB_CASHOUT_WINDOW_SECONDS;
         }
      });
...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
   ...
   if( o.proxy.size() ) {
      ...
      proxy_chain.reserve( COLAB_MAX_PROXY_RECURSION_DEPTH + 1 );
      ...
      while( cprox->proxy.size() != 0 ) {
         ...
         cprox = &next_proxy;
         FC_ASSERT( proxy_chain.size() <= COLAB_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long." );
      }
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
   ...
   if( itr == by_account_witness_idx.end() ) {
      ...
      if ( _db.has_hardfork( COLAB_HARDFORK_0_2 ) )
      {
         FC_ASSERT( voter.witnesses_voted_for < COLAB_MAX_ACCOUNT_WITNESS_VOTES, "Account has voted for too many witnesses." ); // TODO: Remove after hardfork 2
         ...
         if( _db.has_hardfork( COLAB_HARDFORK_0_3 ) ) {
...
void custom_evaluator::do_apply( const custom_operation& o )
{
   ...
   if( _db.is_producing() || _db.has_hardfork( COLAB_HARDFORK_0_20 ) )
   {
      FC_ASSERT( o.required_auths.size() <= COLAB_MAX_AUTHORITY_MEMBERSHIP, "Too many auths specified. Max: 10, Current: ${n}", ("n", o.required_auths.size()) );
   }
}
...
void custom_json_evaluator::do_apply( const custom_json_operation& o )
{
   ...
   if( _db.is_producing() || _db.has_hardfork( COLAB_HARDFORK_0_20 ) )
   {
      ...
      FC_ASSERT( num_auths <= COLAB_MAX_AUTHORITY_MEMBERSHIP, "Too many auths specified. Max: 10, Current: ${n}", ("n", num_auths) );
   }
...
void custom_binary_evaluator::do_apply( const custom_binary_operation& o )
{
   ...
   FC_ASSERT( d.has_hardfork( COLAB_HARDFORK_0_14__317 ) );
...
void pow_apply( database& db, Operation o )
{
   ...
   if( db.has_hardfork( COLAB_HARDFORK_0_5__59 ) )
   {
   ...
   if( db.has_hardfork( COLAB_HARDFORK_0_13__256 ) )
   ...
   if( db.head_block_num() < COLAB_START_MINER_VOTING_BLOCK )
      pow_reward.amount *= COLAB_MAX_WITNESSES;
...
void pow_evaluator::do_apply( const pow_operation& o ) {
   FC_ASSERT( !db().has_hardfork( COLAB_HARDFORK_0_13__256 ), "pow is deprecated. Use pow2 instead" );
   pow_apply( db(), o );
}
...
{
   database& db = this->db();
   FC_ASSERT( !db.has_hardfork( COLAB_HARDFORK_0_17__770 ), "mining is now disabled" );

   const auto& dgp = db.get_dynamic_global_properties();
   uint32_t target_pow = db.get_pow_summary_target();
   account_name_type worker_account;

   if( db.has_hardfork( COLAB_HARDFORK_0_16__551 ) )
   {
   ...
   FC_ASSERT( o.props.maximum_block_size >= COLAB_MIN_BLOCK_SIZE_LIMIT * 2, "Voted maximum block size is too small." );
...
void limit_order_create_evaluator::do_apply( const limit_order_create_operation& o )
{
   ...
   if( _db.has_hardfork( COLAB_HARDFORK_0_20__1449) )
   {
      FC_ASSERT( o.expiration <= _db.head_block_time() + COLAB_MAX_LIMIT_ORDER_EXPIRATION, "Limit Order Expiration must not be more than 28 days in the future" );
   }
   ...
   const auto& order = _db.create<limit_order_object>( [&]( limit_order_object& obj )
   {
       ...
       else
       {
          ...
          obj.expiration = std::min( o.expiration, fc::time_point_sec( COLAB_HARDFORK_0_20_TIME + COLAB_MAX_LIMIT_ORDER_EXPIRATION + rand_offset ) );
       }
   });
...
void limit_order_create2_evaluator::do_apply( const limit_order_create2_operation& o )
{
   ...
   const auto& order = _db.create<limit_order_object>( [&]( limit_order_object& obj )
   {
       ...
       {
          obj.expiration = std::min( o.expiration, fc::time_point_sec( COLAB_HARDFORK_0_20_TIME + COLAB_MAX_LIMIT_ORDER_EXPIRATION ) );
       }
   });
...
{
   FC_ASSERT( !_db.has_hardfork( COLAB_HARDFORK_0_4 ), "report_over_production_operation is disabled." );
}

void claim_account_evaluator::do_apply( const claim_account_operation& o )
{
   FC_ASSERT( _db.has_hardfork( COLAB_HARDFORK_0_20__1771 ), "claim_account_operation is not enabled until hardfork 20." );
   ...
   if( o.fee.amount == 0 )
   {
      ...
      if( !_db.is_pending_tx() )
      {
         ...
         FC_ASSERT( current_witness.available_witness_account_subsidies >= COLAB_ACCOUNT_SUBSIDY_PRECISION, "Witness ${w} does not have enough subsidized accounts to claim",
            ("w", current_witness.owner) );

         _db.modify( current_witness, [&]( witness_object& w )
         {
            w.available_witness_account_subsidies -= COLAB_ACCOUNT_SUBSIDY_PRECISION;
         });
      }

      FC_ASSERT( gpo.available_account_subsidies >= COLAB_ACCOUNT_SUBSIDY_PRECISION, "There are not enough subsidized accounts to claim" );

      _db.modify( gpo, [&]( dynamic_global_property_object& gpo )
      {
         gpo.available_account_subsidies -= COLAB_ACCOUNT_SUBSIDY_PRECISION;
      });
   }
   else
   {
   ...
   _db.adjust_balance( _db.get_account( COLAB_NULL_ACCOUNT ), o.fee );
...
{
   FC_ASSERT( _db.has_hardfork( COLAB_HARDFORK_0_20__1771 ), "create_claimed_account_operation is not enabled until hardfork 20." );
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
   ...
   if ( account_to_recover.recovery_account.length() )   // Make sure recovery matches expected recovery account
   {
      ...
      if( o.recovery_account == COLAB_TEMP_ACCOUNT )
   ...
   if( request == recovery_request_idx.end() ) // New Request
   {
      ...
      _db.create< account_recovery_request_object >( [&]( account_recovery_request_object& req )
      {
         ...
         req.expires = _db.head_block_time() + COLAB_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
      });
   }
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
   ...
   if( _db.has_hardfork( COLAB_HARDFORK_0_12 ) )
      FC_ASSERT( _db.head_block_time() - account.last_account_recovery > COLAB_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
...
void change_recovery_account_evaluator::do_apply( const change_recovery_account_operation& o )
{
   ...
   if( request == change_recovery_idx.end() ) // New request
   {
      _db.create< change_recovery_account_request_object >( [&]( change_recovery_account_request_object& req )
      {
         ...
         req.effective_on = _db.head_block_time() + COLAB_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
...
void transfer_from_savings_evaluator::do_apply( const transfer_from_savings_operation& op )
{
   ...
   FC_ASSERT( from.savings_withdraw_requests < COLAB_SAVINGS_WITHDRAW_REQUEST_LIMIT, "Account has reached limit for pending withdraw requests." );
   ...
   _db.create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
      ...
      s.complete = _db.head_block_time() + COLAB_SAVINGS_WITHDRAW_TIME;
   });
...
{
   FC_ASSERT( _db.has_hardfork( COLAB_HARDFORK_0_14__324 ) );
   ...
   if( o.decline )
   {
      ...
      _db.create< decline_voting_rights_request_object >( [&]( decline_voting_rights_request_object& req )
      {
         ...
         req.effective_date = _db.head_block_time() + COLAB_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
   else
   {
