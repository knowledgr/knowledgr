...
public:
   ...
   /// smt_setup_emissions
   time_point_sec       schedule_time = KNOWLEDGR_GENESIS_TIME;
   knowledgr::protocol::
   ...
   // smt_cap_reveal
   share_type  knowledgr_units_min_cap = -1;
   share_type  knowledgr_units_hard_cap = -1;
};
...
FC_REFLECT_ENUM( knowledgr::chain::smt_phase,
                  (account_elevated)
                  (setup_completed)
                  (contribution_begin_time_completed)
                  (contribution_end_time_completed)
...
FC_REFLECT( knowledgr::chain::smt_token_object,
   (id)
   (liquid_symbol)
   (control_account)
   (phase)
   ...
   (knowledgr_units_min_cap)
   (knowledgr_units_hard_cap)
)

FC_REFLECT( knowledgr::chain::smt_event_token_object,
   (id)
   (parent)
   (phase)
   (generation_begin_time)
...
CHAINBASE_SET_INDEX_TYPE( knowledgr::chain::smt_event_token_object, knowledgr::chain::smt_event_token_index )
