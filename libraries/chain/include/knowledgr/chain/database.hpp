 ...
 */
#pragma once
#include <knowledgr/chain/block_log.hpp>
#include <knowledgr/chain/fork_database.hpp>
#include <knowledgr/chain/global_property_object.hpp>
#include <knowledgr/chain/hardfork_property_object.hpp>
#include <knowledgr/chain/node_property_object.hpp>
#include <knowledgr/chain/notifications.hpp>

#include <knowledgr/chain/util/advanced_benchmark_dumper.hpp>
#include <knowledgr/chain/util/signal.hpp>
   ...
   class database : public chainbase::database
   {
      public:
         ...
         struct open_args
         {
            ...
            uint64_t initial_supply = KNOWLEDGR_INIT_SUPPLY;
         ...
         chain_id_type knowledgr_chain_id = KNOWLEDGR_CHAIN_ID;
...
//         const feed_history_object&             get_feed_history()const;
      ...
      public:
          ...
          *
          * Passing slot_num == 0 returns KNOWLEDGR_NULL_WITNESS
          */
...
//          std::pair< asset, asset > create_sbd( const account_object& to_account, asset knowledgr, bool to_reward_balance=false );
         ...
         void        adjust_liquidity_reward( const account_object& owner, const asset& volume/*, bool is_bid = false*/);
...
//          asset to_knowledgr( const asset& sbd )const;
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = KNOWLEDGR_INIT_SUPPLY );
...
//         void perform_vesting_share_split( uint32_t magnitude );
   ...
   protected:
      ...
      private:
         ...
         fc::time_point_sec            _hardfork_times[ KNOWLEDGR_NUM_HARDFORKS + 1 ];
         protocol::hardfork_version    _hardfork_versions[ KNOWLEDGR_NUM_HARDFORKS + 1 ];
