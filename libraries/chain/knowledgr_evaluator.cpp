...
{
   FC_ASSERT( permlink.size() > KNOWLEDGR_MIN_PERMLINK_LENGTH && permlink.size() < KNOWLEDGR_MAX_PERMLINK_LENGTH, "Permlink is not a valid size." );
...
void witness_update_evaluator::do_apply( const witness_update_operation& o )
{
   ...
   if ( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_14__410 ) )
   {
      ...
      if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__2651 ) || _db.is_producing() )
      {
         ...
         FC_ASSERT( o.props.account_creation_fee.amount <= KNOWLEDGR_MAX_ACCOUNT_CREATION_FEE, "account_creation_fee greater than maximum account creation fee" );
      }
   }
   ...
   if( _db.is_producing() || _db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__2642 ))
   {
      FC_ASSERT( o.props.maximum_block_size <= KNOWLEDGR_SOFT_MAX_BLOCK_SIZE, "Max block size cannot be more than 2MiB" );
   }
...
{
   FC_ASSERT( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__1620 ), "witness_set_properties_evaluator not enabled until HF 20" );
...
//    time_point_sec    last_sbd_exchange_update;
   ...
   if( flags.account_creation_changed )
   {
      ...
      {
         FC_TODO( "Move to validate() after HF20" );
         FC_ASSERT( props.account_creation_fee.amount <= KNOWLEDGR_MAX_ACCOUNT_CREATION_FEE, "account_creation_fee greater than maximum account creation fee" );
      }
   }
   ...
   const dynamic_global_property_object& props, bool mined, const account_name_type& recovery_account, uint32_t hardfork )
{
   ...
   if( hardfork >= KNOWLEDGR_HARDFORK_0_11 ) {
      FC_TODO( "If after HF 20, there are no temp account creations, the HF check can be removed." )
      if( ( hardfork < KNOWLEDGR_HARDFORK_0_20__1782 ) || ( recovery_account != KNOWLEDGR_TEMP_ACCOUNT ) )
      {
         acc.recovery_account = recovery_account;
      }
   } else {
      acc.recovery_account = "knowledgr";
   }
}
...
void stake_process_evaluator::do_apply( const stake_process_operation& o )
{
	...
	FC_ASSERT( admin.member_of == account_object::admin || (std::string)admin.name == KNOWLEDGR_INIT_MINER_NAME, "Processing pending stakes can only be done by admin member." );
...
{
	const auto& admin = _db.get_account( o.admin);
	FC_ASSERT( admin.member_of == account_object::admin || (std::string)admin.name == KNOWLEDGR_INIT_MINER_NAME, "Allocating expertise rate for an account can only be done by admin member." );
	const auto& account = _db.get_account( o.account );
...
void account_expertise_update_evaluator::do_apply( const account_expertise_update_operation& o )
{
	...
	_db.modify( account, [&]( account_object& acc ) {
		for (auto & d0 : o.expertises) {
			if (d0.category == knowledgr::protocol::expctgry_unknown) continue;
			bool _exist = false;
...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
   ...
   if( _db.is_producing() || _db.has_hardfork( KNOWLEDGR_HARDFORK_0_20 ) )
   {
   ...
   if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_15__465 ) )
   {
   ...
   if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__1762 ) )
   {
      _db.adjust_balance( _db.get< account_object, by_name >( KNOWLEDGR_NULL_ACCOUNT ), o.fee );
   }
   ...
   const auto& new_account = _db.create< account_object >( [&]( account_object& acc )
   {
      initialize_account_object( acc, o.new_account_name, o.memo_key, props, false /*mined*/, o.creator, _db.get_hardfork() );
		...
		if (props.num_of_accounts < KNOWLEDGR_NUM_OF_INIT_STAKING_ACCOUNTS) {
			...
			{
				init0.balance -= KNOWLEDGR_LIMIT_STAKING_AMOUNT;
			});
		}
   });
   ...
   if( !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__1762 ) && o.fee.amount > 0 )
   {
...
// {
//    FC_ASSERT( !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__1760 ), "Account creation with delegation is deprecated as of Hardfork 20" );
// 
//    if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__2651 ) || _db.is_producing() )
//    {
//       FC_TODO( "Move to validate() after HF20" );
...
//    });
// 
//    if( o.delegation.amount > 0 || !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_19__997 ) )
//    {
...
//          vdo.min_delegation_time = _db.head_block_time() + KNOWLEDGR_CREATE_ACCOUNT_DELEGATION_TIME;
//       });
//    }
// 
//    if( !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__1762 ) && o.fee.amount > 0 )
...
{
   if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_1 ) ) FC_ASSERT( o.account != KNOWLEDGR_TEMP_ACCOUNT, "Cannot update temp account." );

   if( ( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_15__465 ) ) && o.posting )
      o.posting->validate();
   ...
   {
#ifndef IS_TEST_NET
      if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_11 ) )
         FC_ASSERT( _db.head_block_time() - account_auth.last_owner_update > KNOWLEDGR_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
#endif
   ...
   if( o.active && ( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_15__465 ) ) )
      verify_authority_accounts_exist( _db, *o.active, o.account, authority::active );
   if( o.posting && ( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_15__465 ) ) )
...
void delete_comment_evaluator::do_apply( const delete_comment_operation& o )
{
   ...
   if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_19__876 ) )
      FC_ASSERT( comment.cashout_time != fc::time_point_sec::maximum() );

   if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_19__977 ) )
   ...
   if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_6__80 ) && comment.parent_author != KNOWLEDGR_ROOT_POST_PARENT )
   {
...
void comment_options_evaluator::do_apply( const comment_options_operation& o )
{
   ...
   FC_ASSERT( comment.percent_knowledgr_dollars >= o.percent_knowledgr_dollars, "A comment cannot accept a greater percent SBD." );

   _db.modify( comment, [&]( comment_object& c ) {
       c.max_accepted_payout   = o.max_accepted_payout;
       c.percent_knowledgr_dollars = o.percent_knowledgr_dollars;
...
{ try {
   ...
   if( o.parent_author != KNOWLEDGR_ROOT_POST_PARENT )
   {
      ...
      if( !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_17__767 ) )
         FC_ASSERT( parent->depth < KNOWLEDGR_MAX_COMMENT_DEPTH_PRE_HF17, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",KNOWLEDGR_MAX_COMMENT_DEPTH_PRE_HF17) );
      else
         FC_ASSERT( parent->depth < KNOWLEDGR_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",KNOWLEDGR_MAX_COMMENT_DEPTH) );
   }
   ...
   if ( itr == by_permlink_idx.end() )
   {
      ...
      uint16_t reward_weight = KNOWLEDGR_100_PERCENT;
      uint64_t post_bandwidth = auth.post_bandwidth;

      if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_12__176 ) && !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_17__733 ) && o.parent_author == KNOWLEDGR_ROOT_POST_PARENT )
      {
         uint64_t post_delta_time = std::min( _db.head_block_time().sec_since_epoch() - auth.last_root_post.sec_since_epoch(), KNOWLEDGR_POST_AVERAGE_WINDOW );
         uint32_t old_weight = uint32_t( ( post_bandwidth * ( KNOWLEDGR_POST_AVERAGE_WINDOW - post_delta_time ) ) / KNOWLEDGR_POST_AVERAGE_WINDOW );
         post_bandwidth = ( old_weight + KNOWLEDGR_100_PERCENT );
         reward_weight = uint16_t( std::min( ( KNOWLEDGR_POST_WEIGHT_CONSTANT * KNOWLEDGR_100_PERCENT ) / ( post_bandwidth * post_bandwidth ), uint64_t( KNOWLEDGR_100_PERCENT ) ) );
      }
      ...
      {
         if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_1 ) )
         {
         ...
         {
            com.cashout_time = com.created + KNOWLEDGR_CASHOUT_WINDOW_SECONDS;
         }
      });
   ...
   else // start edit case
   {
      ...
      _db.modify( comment, [&]( comment_object& com )
      {
         ...
         if( !parent )
         {
            ...
            FC_ASSERT( to_string(com.parent_permlink) == "knowledgr", "The permlink of a comment cannot change." );
         }
         else
         {
...
// 
// 
//    FC_ASSERT( account.vesting_shares >= asset( 0, VESTS_SYMBOL ), "Account does not have sufficient Knowledgr Power for withdraw." );
//    FC_ASSERT( account.vesting_shares - account.delegated_vesting_shares >= o.vesting_shares, "Account does not have sufficient Knowledgr Power for withdraw." );
// 
//    FC_TODO( "Remove this entire block after HF 20" )
//    if( !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__1860 ) && !account.mined && _db.has_hardfork( KNOWLEDGR_HARDFORK_0_1 ) )
//    {
//       const auto& props = _db.get_dynamic_global_properties();
//       const witness_schedule_object& wso = _db.get_witness_schedule_object();
// 
...
// 
//       FC_ASSERT( account.vesting_shares > min_vests || ( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_16__562 ) && o.vesting_shares.amount == 0 ),
//                  "Account registered by another account requires 10x account creation fee worth of Knowledgr Power before it can be powered down." );
//    }
// 
//    if( o.vesting_shares.amount == 0 )
//    {
//       if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_5__57 ) )
...
//    else
//    {
//       int vesting_withdraw_intervals = KNOWLEDGR_VESTING_WITHDRAW_INTERVALS_PRE_HF_16;
//       if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_16__551 ) )
//          vesting_withdraw_intervals = KNOWLEDGR_VESTING_WITHDRAW_INTERVALS; /// 13 weeks = 1 quarter of a year
// 
//       _db.modify( account, [&]( account_object& a )
//       {
...
//          a.next_vesting_withdrawal = _db.head_block_time() + fc::seconds(KNOWLEDGR_VESTING_WITHDRAW_INTERVAL_SECONDS);
...
//       FC_ASSERT( from_account.withdraw_routes < KNOWLEDGR_MAX_WITHDRAW_ROUTES, "Account already has the maximum number of routes." );
// 
...
//    }
// 
//    FC_ASSERT( total_percent <= KNOWLEDGR_100_PERCENT, "More than 100% of vesting withdrawals allocated to destinations." );
//    }
...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
   ...
   if( o.proxy.size() ) {
      ...
      proxy_chain.reserve( KNOWLEDGR_MAX_PROXY_RECURSION_DEPTH + 1 );
      ...
      while( cprox->proxy.size() != 0 ) {
         ...
         cprox = &next_proxy;
         FC_ASSERT( proxy_chain.size() <= KNOWLEDGR_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long." );
      }
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
   ...
   if( itr == by_account_witness_idx.end() ) {
      ...
      if ( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_2 ) )
      {
         FC_ASSERT( voter.witnesses_voted_for < KNOWLEDGR_MAX_ACCOUNT_WITNESS_VOTES, "Account has voted for too many witnesses." ); // TODO: Remove after hardfork 2
         ...
         if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_3 ) ) {
...
uint128_t calculate_voter_rshare_factor(const account_object& voter, const comment_object& comment)
{
	...
	X = ( stake*KNOWLEDGR_1_PERCENT*25 + rep*KNOWLEDGR_1_PERCENT*75 );
	X *= static_cast<long double>(MAX_RSHARE_FACTOR);
	X /= static_cast<long double>(KNOWLEDGR_100_PERCENT);
...
void knowledgr_vote_evaluator( const vote_operation& o, database& _db )
{
   const auto& comment = _db.get_comment( o.author, o.permlink );
   ...
   auto now = _db.head_block_time();
   FC_ASSERT( ( now - voter.last_vote_time ).to_seconds() >= KNOWLEDGR_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );
   ...
   std::cerr<<"~~~ [knowledgr_vote_evaluator()] - abs_weight = "<<abs_weight<<"\n";
   ...
   std::cerr<<"~~~ [knowledgr_vote_evaluator()] - used_mana = "<<used_mana.to_uint64()<<"\n";
   ...
   std::cerr<<"~~~ [knowledgr_vote_evaluator()] - rshare_factor = "<<(std::string)rshare_factor<<"\n";

   int64_t abs_rshares = (used_mana * rshare_factor / KNOWLEDGR_100_PERCENT).to_uint64() ;

   std::cerr<<"~~~ [knowledgr_vote_evaluator()] - abs_rshares = "<<abs_rshares<<"\n";

#if 0/// no need here
   uint32_t cashout_delta = ( comment.cashout_time - _db.head_block_time() ).to_seconds();
   if( cashout_delta < KNOWLEDGR_UPVOTE_LOCKOUT_SECONDS )
   {
      abs_rshares = (int64_t) ( ( uint128_t( abs_rshares ) * cashout_delta ) / KNOWLEDGR_UPVOTE_LOCKOUT_SECONDS ).to_uint64();
   }
#endif///
   ...
   {
      FC_ASSERT( itr->num_changes < KNOWLEDGR_MAX_VOTE_CHANGES, "Voter has used the maximum number of vote changes on this comment." );
...
// 
//    int16_t abs_weight = abs( o.weight );
//    uint128_t used_mana = ( uint128_t( voter.voting_manabar.current_mana ) * abs_weight * 60 * 60 * 24 ) / KNOWLEDGR_100_PERCENT;
// 
//    const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();
// 
//    int64_t max_vote_denom = dgpo.vote_power_reserve_rate * KNOWLEDGR_VOTING_MANA_REGENERATION_SECONDS;
...
// 
//    abs_rshares -= KNOWLEDGR_VOTE_DUST_THRESHOLD;
...
//    if( o.weight > 0 ) FC_ASSERT( comment.allow_votes, "Votes are not allowed on the comment." );
// 
//    if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_12__177 ) && _db.calculate_discussion_payout_time( comment ) == fc::time_point_sec::maximum() )
//    {
// #ifndef CLEAR_VOTES
//       const auto& comment_vote_idx = _db.get_index< comment_vote_index >().indices().get< by_comment_voter >();
//       auto itr = comment_vote_idx.find( std::make_tuple( comment.id, voter.id ) );
...
// 
//    if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_11 ) )
//       FC_ASSERT( elapsed_seconds >= KNOWLEDGR_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );
// 
//    int64_t regenerated_power = (KNOWLEDGR_100_PERCENT * elapsed_seconds) / KNOWLEDGR_VOTING_MANA_REGENERATION_SECONDS;
//    int64_t current_power     = std::min( int64_t(voter.voting_manabar.current_mana) + regenerated_power, int64_t(KNOWLEDGR_100_PERCENT) );
...
//    int64_t  used_power  = ((current_power * abs_weight) / KNOWLEDGR_100_PERCENT) * (60*60*24);
// 
//    const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();
// 
...
//    FC_ASSERT( max_vote_denom > 0 );
// 
//    if( !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_14__259 ) )
//    {
...
// 
//    int64_t abs_rshares    = ((uint128_t( _db.get_effective_vesting_shares( voter, VESTS_SYMBOL ).amount.value ) * used_power) / (KNOWLEDGR_100_PERCENT)).to_uint64();
//    if( !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_14__259 ) && abs_rshares == 0 ) abs_rshares = 1;
// 
//    if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__1764 ) )
//    {
//       abs_rshares -= KNOWLEDGR_VOTE_DUST_THRESHOLD;
//       abs_rshares = std::max( int64_t(0), abs_rshares );
//    }
//    else if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_14__259 ) )
//    {
//       FC_ASSERT( abs_rshares > KNOWLEDGR_VOTE_DUST_THRESHOLD || o.weight == 0, "Voting weight is too small, please accumulate more voting power or knowledgr power." );
//    }
//    else if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_13__248 ) )
//    {
//       FC_ASSERT( abs_rshares > KNOWLEDGR_VOTE_DUST_THRESHOLD || abs_rshares == 1, "Voting weight is too small, please accumulate more voting power or knowledgr power." );
//    }
// 
//    // Lazily delete vote
//    if( itr != comment_vote_idx.end() && itr->num_changes == -1 )
//    {
//       if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_12__177 ) )
//          FC_ASSERT( false, "Cannot vote again on a comment after payout." );
// 
//       _db.remove( *itr );
//       itr = comment_vote_idx.end();
...
//       if( rshares > 0 )
//       {
//          if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_17__900 ) )
//             FC_ASSERT( _db.head_block_time() < comment.cashout_time - KNOWLEDGR_UPVOTE_LOCKOUT_HF17, "Cannot increase payout within last twelve hours before payout." );
//          else if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_7 ) )
//             FC_ASSERT( _db.head_block_time() < _db.calculate_discussion_payout_time( comment ) - KNOWLEDGR_UPVOTE_LOCKOUT_HF7, "Cannot increase payout within last minute before payout." );
//       }
// 
...
// 
//          if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_12__177 ) && !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_13__257)  )
//             new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + KNOWLEDGR_CASHOUT_WINDOW_SECONDS_PRE_HF17;
//          else
//             new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + KNOWLEDGR_CASHOUT_WINDOW_SECONDS_PRE_HF12;
// 
...
//          else
//             c.net_votes--;
//          if( !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_6__114 ) && c.net_rshares == -c.abs_rshares) FC_ASSERT( c.net_votes < 0, "Comment has negative net votes?" );
//       });
// 
//       _db.modify( root, [&]( comment_object& c )
//       {
...
//          if( !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_17__769 ) )
//          {
//             if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_12__177 ) && c.last_payout > fc::time_point_sec::min() )
...
//                c.max_cashout_time = _db.head_block_time() + fc::seconds( KNOWLEDGR_MAX_CASHOUT_WINDOW_SECONDS );
//          }
//       });
// 
//       fc::uint128_t new_rshares = std::max( comment.net_rshares.value, int64_t(0));
...
//          bool curation_reward_eligible = rshares > 0 && (comment.last_payout == fc::time_point_sec()) && comment.allow_curation_rewards;
// 
//          if( curation_reward_eligible && _db.has_hardfork( KNOWLEDGR_HARDFORK_0_17__774 ) )
//             curation_reward_eligible = _db.get_curation_rewards_percent( comment ) > 0;
// 
//          if( curation_reward_eligible )
//          {
//             if( comment.created < fc::time_point_sec(KNOWLEDGR_HARDFORK_0_6_REVERSE_AUCTION_TIME) ) {
...
//                {
//                   const auto& reward_fund = _db.get_reward_fund( comment );
//                   auto curve = !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_19__1052 ) && comment.created > KNOWLEDGR_HF_19_SQRT_PRE_CALC
...
//             max_vote_weight = cv.weight;
// 
//             if( _db.head_block_time() > fc::time_point_sec(KNOWLEDGR_HARDFORK_0_6_REVERSE_AUCTION_TIME) )  /// start enforcing this prior to the hardfork
//             {
//                /// discount weight by time
//                uint128_t w(max_vote_weight);
...
//       FC_ASSERT( itr->num_changes < KNOWLEDGR_MAX_VOTE_CHANGES, "Voter has used the maximum number of vote changes on this comment." );
// 
//       if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_6__112 ) )
...
//             new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + KNOWLEDGR_CASHOUT_WINDOW_SECONDS_PRE_HF12;
// 
//          if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_14__259 ) && abs_rshares == 0 )
...
void custom_evaluator::do_apply( const custom_operation& o )
{
   ...
   if( _db.is_producing() || _db.has_hardfork( KNOWLEDGR_HARDFORK_0_20 ) )
   {
      FC_ASSERT( o.required_auths.size() <= KNOWLEDGR_MAX_AUTHORITY_MEMBERSHIP, "Too many auths specified. Max: 10, Current: ${n}", ("n", o.required_auths.size()) );
   }
}
...
void custom_json_evaluator::do_apply( const custom_json_operation& o )
{
   ...
   if( _db.is_producing() || _db.has_hardfork( KNOWLEDGR_HARDFORK_0_20 ) )
   {
      ...
      FC_ASSERT( num_auths <= KNOWLEDGR_MAX_AUTHORITY_MEMBERSHIP, "Too many auths specified. Max: 10, Current: ${n}", ("n", num_auths) );
   }
...
void custom_binary_evaluator::do_apply( const custom_binary_operation& o )
{
   ...
   FC_ASSERT( d.has_hardfork( KNOWLEDGR_HARDFORK_0_14__317 ) );
...
void pow_apply( database& db, Operation o )
{
   ...
   if( db.has_hardfork( KNOWLEDGR_HARDFORK_0_5__59 ) )
   {
   ...
   if( db.has_hardfork( KNOWLEDGR_HARDFORK_0_13__256 ) )
   ...
   if( db.head_block_num() < KNOWLEDGR_START_MINER_VOTING_BLOCK )
      pow_reward.amount *= KNOWLEDGR_MAX_WITNESSES;
...
void pow_evaluator::do_apply( const pow_operation& o ) {
   FC_ASSERT( !db().has_hardfork( KNOWLEDGR_HARDFORK_0_13__256 ), "pow is deprecated. Use pow2 instead" );
   pow_apply( db(), o );
}
...
{
   database& db = this->db();
   FC_ASSERT( !db.has_hardfork( KNOWLEDGR_HARDFORK_0_17__770 ), "mining is now disabled" );

   const auto& dgp = db.get_dynamic_global_properties();
   uint32_t target_pow = db.get_pow_summary_target();
   account_name_type worker_account;

   if( db.has_hardfork( KNOWLEDGR_HARDFORK_0_16__551 ) )
   {
   ...
   FC_ASSERT( o.props.maximum_block_size >= KNOWLEDGR_MIN_BLOCK_SIZE_LIMIT * 2, "Voted maximum block size is too small." );
...
// 
//   auto knowledgr_conversion_delay = KNOWLEDGR_CONVERSION_DELAY_PRE_HF_16;
//   if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_16__551) )
//      knowledgr_conversion_delay = KNOWLEDGR_CONVERSION_DELAY;
// 
...
//       obj.amount          = o.amount;
//       obj.conversion_date = _db.head_block_time() + knowledgr_conversion_delay;
//   });
// 
// }
...
void limit_order_create_evaluator::do_apply( const limit_order_create_operation& o )
{
   ...
   if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__1449) )
   {
      FC_ASSERT( o.expiration <= _db.head_block_time() + KNOWLEDGR_MAX_LIMIT_ORDER_EXPIRATION, "Limit Order Expiration must not be more than 28 days in the future" );
   }
   ...
   const auto& order = _db.create<limit_order_object>( [&]( limit_order_object& obj )
   {
       ...
       else
       {
          ...
          obj.expiration = std::min( o.expiration, fc::time_point_sec( KNOWLEDGR_HARDFORK_0_20_TIME + KNOWLEDGR_MAX_LIMIT_ORDER_EXPIRATION + rand_offset ) );
       }
   });
...
void limit_order_create2_evaluator::do_apply( const limit_order_create2_operation& o )
{
   ...
   const auto& order = _db.create<limit_order_object>( [&]( limit_order_object& obj )
   {
       ...
       {
          obj.expiration = std::min( o.expiration, fc::time_point_sec( KNOWLEDGR_HARDFORK_0_20_TIME + KNOWLEDGR_MAX_LIMIT_ORDER_EXPIRATION ) );
       }
   });
...
{
   FC_ASSERT( !_db.has_hardfork( KNOWLEDGR_HARDFORK_0_4 ), "report_over_production_operation is disabled." );
}

void claim_account_evaluator::do_apply( const claim_account_operation& o )
{
   FC_ASSERT( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__1771 ), "claim_account_operation is not enabled until hardfork 20." );
   ...
   if( o.fee.amount == 0 )
   {
      ...
      if( !_db.is_pending_tx() )
      {
         ...
         FC_ASSERT( current_witness.available_witness_account_subsidies >= KNOWLEDGR_ACCOUNT_SUBSIDY_PRECISION, "Witness ${w} does not have enough subsidized accounts to claim",
            ("w", current_witness.owner) );

         _db.modify( current_witness, [&]( witness_object& w )
         {
            w.available_witness_account_subsidies -= KNOWLEDGR_ACCOUNT_SUBSIDY_PRECISION;
         });
      }

      FC_ASSERT( gpo.available_account_subsidies >= KNOWLEDGR_ACCOUNT_SUBSIDY_PRECISION, "There are not enough subsidized accounts to claim" );

      _db.modify( gpo, [&]( dynamic_global_property_object& gpo )
      {
         gpo.available_account_subsidies -= KNOWLEDGR_ACCOUNT_SUBSIDY_PRECISION;
      });
   }
   else
   {
   ...
   _db.adjust_balance( _db.get_account( KNOWLEDGR_NULL_ACCOUNT ), o.fee );
...
{
   FC_ASSERT( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_20__1771 ), "create_claimed_account_operation is not enabled until hardfork 20." );
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
   ...
   if ( account_to_recover.recovery_account.length() )   // Make sure recovery matches expected recovery account
   {
      ...
      if( o.recovery_account == KNOWLEDGR_TEMP_ACCOUNT )
   ...
   if( request == recovery_request_idx.end() ) // New Request
   {
      ...
      _db.create< account_recovery_request_object >( [&]( account_recovery_request_object& req )
      {
         ...
         req.expires = _db.head_block_time() + KNOWLEDGR_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
      });
   }
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
   ...
   if( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_12 ) )
      FC_ASSERT( _db.head_block_time() - account.last_account_recovery > KNOWLEDGR_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
...
void change_recovery_account_evaluator::do_apply( const change_recovery_account_operation& o )
{
   ...
   if( request == change_recovery_idx.end() ) // New request
   {
      _db.create< change_recovery_account_request_object >( [&]( change_recovery_account_request_object& req )
      {
         ...
         req.effective_on = _db.head_block_time() + KNOWLEDGR_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
...
void transfer_from_savings_evaluator::do_apply( const transfer_from_savings_operation& op )
{
   ...
   FC_ASSERT( from.savings_withdraw_requests < KNOWLEDGR_SAVINGS_WITHDRAW_REQUEST_LIMIT, "Account has reached limit for pending withdraw requests." );
   ...
   _db.create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
      ...
      s.complete = _db.head_block_time() + KNOWLEDGR_SAVINGS_WITHDRAW_TIME;
   });
...
{
   FC_ASSERT( _db.has_hardfork( KNOWLEDGR_HARDFORK_0_14__324 ) );
   ...
   if( o.decline )
   {
      ...
      _db.create< decline_voting_rights_request_object >( [&]( decline_voting_rights_request_object& req )
      {
         ...
         req.effective_date = _db.head_block_time() + KNOWLEDGR_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
   else
   {
...
//   _db.adjust_reward_balance( acnt, -op.reward_sbd );
   _db.adjust_balance( acnt, op.reward_knowledgr );
...
//       {
//          util::manabar_params params( max_mana, KNOWLEDGR_VOTING_MANA_REGENERATION_SECONDS );
//          a.voting_manabar.regenerate_mana( params, _db.head_block_time() );
//       });
// 
...
// 
//       FC_ASSERT( delta >= min_update, "Knowledgr Power increase is not enough of a difference. min_update: ${min}", ("min", min_update) );
...
//       {
//          FC_ASSERT( delta >= min_update, "Knowledgr Power decrease is not enough of a difference. min_update: ${min}", ("min", min_update) );
