...
{
   public:
      tags_api_impl() : _db( appbase::app().get_plugin< colab::plugins::chain::chain_plugin >().db() ) {}
      ...
      chain::database& _db;
      std::shared_ptr< colab::plugins::follow::follow_api > _follow_api;
};
...
DEFINE_API_IMPL( tags_api_impl, get_discussions_by_promoted )
{
   ...
   auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type( COLAB_MAX_SHARE_SUPPLY ) )  );
...
{
   JSON_RPC_REGISTER_API( COLAB_TAGS_API_PLUGIN_NAME );
}
...
{
   auto follow_api_plugin = appbase::app().find_plugin< colab::plugins::follow::follow_api_plugin >();
