#pragma once

#include <fc/time.hpp>

#include <knowledgr/chain/database.hpp>

namespace knowledgr { namespace plugins { namespace chain {
   
class abstract_block_producer {
public:
   virtual ~abstract_block_producer() = default;

   virtual knowledgr::chain::signed_block generate_block(
      fc::time_point_sec when,
      const knowledgr::chain::account_name_type& witness_owner,
      const fc::ecc::private_key& block_signing_private_key,
      uint32_t skip = knowledgr::chain::database::skip_nothing) = 0;
};

} } } // knowledgr::plugins::chain
