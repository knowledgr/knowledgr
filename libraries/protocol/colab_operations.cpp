...
namespace colab { namespace protocol {
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries[0].weight <= COLAB_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= COLAB_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= COLAB_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   void claim_account_operation::validate()const
   {
      ...
      FC_ASSERT( is_asset_type( fee, CLC_SYMBOL ), "Account creation fee must be CLC" );
      FC_ASSERT( fee >= asset( 0, CLC_SYMBOL ), "Account creation fee cannot be negative" );
      FC_ASSERT( fee <= asset( COLAB_MAX_ACCOUNT_CREATION_FEE, CLC_SYMBOL ), "Account creation fee cannot be too large" );
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= COLAB_100_PERCENT, "Weight is not a COLABIT percentage" );
      validate_permlink( permlink );
   }
   ...
   { try {
      ...
      FC_ASSERT( amount.symbol != VESTS_SYMBOL, "transferring of Colab Power (STMP) is not allowed." );
      FC_ASSERT( amount.amount > 0, "Cannot transfer a negative amount (aka: stealing)" );
      FC_ASSERT( memo.size() < COLAB_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( url.size() <= COLAB_MAX_WITNESS_URL_LENGTH, "URL is too long" );

      FC_ASSERT( url.size() > 0, "URL size must be greater than 0" );
      FC_ASSERT( fc::is_utf8( url ), "URL is not valid UTF8" );
      FC_ASSERT( fee >= asset( 0, CLC_SYMBOL ), "Fee cannot be negative" );
   ...
   void witness_set_properties_operation::validate() const
   {
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_creation_fee.symbol == CLC_SYMBOL, "account_creation_fee must be in CLC" );
         FC_ASSERT( account_creation_fee.amount >= COLAB_MIN_ACCOUNT_CREATION_FEE, "account_creation_fee smaller than minimum account creation fee" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_budget >= COLAB_RD_MIN_BUDGET, "Budget must be at least ${n}", ("n", COLAB_RD_MIN_BUDGET) );
         FC_ASSERT( account_subsidy_budget <= COLAB_RD_MAX_BUDGET, "Budget must be at most ${n}", ("n", COLAB_RD_MAX_BUDGET) );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_decay >= COLAB_RD_MIN_DECAY, "Decay must be at least ${n}", ("n", COLAB_RD_MIN_DECAY) );
         FC_ASSERT( account_subsidy_decay <= COLAB_RD_MAX_DECAY, "Decay must be at most ${n}", ("n", COLAB_RD_MAX_DECAY) );
      }
   }
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( COLAB_EQUIHASH_N, COLAB_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == COLAB_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == COLAB_EQUIHASH_K, "proof of work 'k' value is incorrect" );
   ...
   void feed_publish_operation::validate()const
   {
      ...
      FC_ASSERT( ( is_asset_type( exchange_rate.base, CLC_SYMBOL ) && is_asset_type( exchange_rate.quote, SBD_SYMBOL ) )
         || ( is_asset_type( exchange_rate.base, SBD_SYMBOL ) && is_asset_type( exchange_rate.quote, CLC_SYMBOL ) ),
         "Price feed must be a CLC/SBD price" );
   ...
   void limit_order_create_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, CLC_SYMBOL ) && is_asset_type( min_to_receive, SBD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, SBD_SYMBOL ) && is_asset_type( min_to_receive, CLC_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
                     && is_asset_type( min_to_receive, CLC_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, CLC_SYMBOL )
                     && min_to_receive.symbol.space() == asset_symbol_type::smt_nai_space
                  ),
               "Limit order must be for the CLC:SBD or SMT:(CLC/SBD) market" );
   ...
   void limit_order_create2_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, CLC_SYMBOL ) && is_asset_type( exchange_rate.quote, SBD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, SBD_SYMBOL ) && is_asset_type( exchange_rate.quote, CLC_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
                     && is_asset_type( exchange_rate.quote, CLC_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, CLC_SYMBOL )
   ...
   void transfer_to_savings_operation::validate()const {
      ...
      FC_ASSERT( amount.symbol == CLC_SYMBOL || amount.symbol == SBD_SYMBOL );
      FC_ASSERT( memo.size() < COLAB_MAX_MEMO_SIZE, "Memo is too large" );
      FC_ASSERT( fc::is_utf8( memo ), "Memo is not UTF8" );
   }
