...
//    }
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries[0].weight <= KNOWLEDGR_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= KNOWLEDGR_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= KNOWLEDGR_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= KNOWLEDGR_100_PERCENT, "Weight is not a KNOWLEDGRIT percentage" );
      validate_permlink( permlink );
   }
   ...
   { try {
      ...
      FC_ASSERT( amount.symbol != VESTS_SYMBOL, "transferring of Knowledgr Power (STMP) is not allowed." );
...
//       FC_ASSERT( 0 <= percent && percent <= KNOWLEDGR_100_PERCENT, "Percent must be valid knowledgr percent" );
//    }
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( url.size() <= KNOWLEDGR_MAX_WITNESS_URL_LENGTH, "URL is too long" );
   ...
   void witness_set_properties_operation::validate() const
   {
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( maximum_block_size >= KNOWLEDGR_MIN_BLOCK_SIZE_LIMIT, "maximum_block_size smaller than minimum max block size" );
      }
...
//          FC_ASSERT( sbd_interest_rate <= KNOWLEDGR_100_PERCENT, "sbd_interest_rate must not exceed 100%" );
//       }
...
//       }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_budget >= KNOWLEDGR_RD_MIN_BUDGET, "Budget must be at least ${n}", ("n", KNOWLEDGR_RD_MIN_BUDGET) );
         FC_ASSERT( account_subsidy_budget <= KNOWLEDGR_RD_MAX_BUDGET, "Budget must be at most ${n}", ("n", KNOWLEDGR_RD_MAX_BUDGET) );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_decay >= KNOWLEDGR_RD_MIN_DECAY, "Decay must be at least ${n}", ("n", KNOWLEDGR_RD_MIN_DECAY) );
         FC_ASSERT( account_subsidy_decay <= KNOWLEDGR_RD_MAX_DECAY, "Decay must be at most ${n}", ("n", KNOWLEDGR_RD_MAX_DECAY) );
      }
   }
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( KNOWLEDGR_EQUIHASH_N, KNOWLEDGR_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == KNOWLEDGR_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == KNOWLEDGR_EQUIHASH_K, "proof of work 'k' value is incorrect" );
...
//       FC_ASSERT( reward_vests.amount >= 0, "Cannot claim a negative amount" );
      FC_ASSERT( reward_knowledgr.amount > 0/* || reward_sbd.amount > 0 || reward_vests.amount > 0*/, "Must claim something." );
   }
