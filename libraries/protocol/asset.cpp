...
{
   static_assert( COLAB_ASSET_SYMBOL_NAI_STRING_LENGTH >= 12, "This code will overflow a short buffer" );
   uint32_t x = to_nai();
   buf[11] = '\0';
   buf[10] = ((x%10)+'0');  x /= 10;
   buf[ 9] = ((x%10)+'0');  x /= 10;
...
uint32_t asset_symbol_type::asset_num_from_nai( uint32_t nai, uint8_t decimal_places )
{
   ...
   switch( nai_data_digits )
   {
      ...
      case COLAB_NAI_SBD:
         FC_ASSERT( decimal_places == COLAB_PRECISION_SBD );
         return COLAB_ASSET_NUM_SBD;
...
void asset_symbol_type::validate()const
{
   switch( asset_num )
   {
      ...
      default:
      {
         uint32_t nai_data_digits = (asset_num >> COLAB_NAI_SHIFT);
...
void asset::validate()const
{
   ...
   FC_ASSERT( amount.value <= COLAB_MAX_SATOSHIS );
}
...
DEFINE_PRICE_COMPARISON_OPERATOR( >= )
      ...
      price price::max( asset_symbol_type base, asset_symbol_type quote ) { return asset( share_type(COLAB_MAX_SATOSHIS), base ) / asset( share_type(1), quote); }
      price price::min( asset_symbol_type base, asset_symbol_type quote ) { return asset( 1, base ) / asset( COLAB_MAX_SATOSHIS, quote); }
...
namespace fc {
   ...
   void from_variant( const fc::variant& var, colab::protocol::asset& vo )
   {
      try
      {
         ...
         vo.symbol = colab::protocol::asset_symbol_type::from_nai_string( v_object[ ASSET_NAI_KEY ].as< std::string >().c_str(), v_object[ ASSET_PRECISION_KEY ].as< uint8_t >() );
      } FC_CAPTURE_AND_RETHROW()
   }
}
