...
of a running Colab Debug Node. The Debug Node is a plugin for Colab that allows realtime
...
[Python Colab library](https://github.com/xeroc/python-colablib).
...
To use the script include `from colabdebugnode import DebugNode`

There are a couple of examples already made that you can try modifying yourself.

[debug_hardforks.py](https://github.com/norestlabs/colab/python_scripts/tests/debug_hardforks.py)
...
[debugnode.py](https://github.com/norestlabs/colab/python_scripts/colabdebugnode/debugnode.py#L212)
...
``` Python
debug_node = DebugNode( str( colabd ), str( data_dir ) )
with debug_node:
...
specify the colabd binary location and a populated data directory. For most configurations
this will be `programs/colabd/colabd` and `witness_node_data_dir` respectively, from the
git root directory for Colab.
